...
namespace Astral {
	...
	template<typename T>
	inline void GLUniformWrite(GLuint handle, std::span<const T> data) {
	...
	template<> \
	inline void GLUniformWrite<cpp_type>(GLuint handle, std::span<const cpp_type> data) { \
		AST_CORE_ASSERT(!data.empty(), "Cannot write empty uniform data"); \
		gl_write_func(handle, data.size(), __VA_ARGS__); \
	}
