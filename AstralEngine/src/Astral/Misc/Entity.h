...
namespace Astral {
	...
	public:
		...
		template <std::derived_from<Components::AComponent> ComponentT>
		class ComponentIterator {
		public:
			ComponentIterator(
				std::vector<Box<Components::AComponent>>::iterator components_begin,
				std::vector<Box<Components::AComponent>>::iterator components_end);
		...
		private:
			std::vector<Box<Components::AComponent>>::iterator component_it;
			const std::vector<Box<Components::AComponent>>::iterator components_end;
	...
	public:
		...
		void AddComponent(Box<Components::AComponent> component);
	...
	private:
		...
		std::vector<Box<Components::AComponent>> components;
	...
	inline Entity::Entity(std::string name) :
		parent(nullptr),
		name(name) {
		AddComponent(std::make_unique<Components::Transform>());
	}
	...
	inline Entity* Entity::Child(std::string name) {
		...
		if (it == children.end()) {
			AST_CORE_ASSERT(false, "Child with name {0} not found", name);
			return nullptr;
		}
	...
	inline void Entity::AddComponent(Box<Components::AComponent> component) {
	...
	inline Components::Transform& Entity::transform() {
