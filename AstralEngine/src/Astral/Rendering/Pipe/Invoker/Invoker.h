...
namespace Astral::Render {
	...
	public:
		...
		void Draw(ResourceHandle shader, std::vector<ResourceBinding> bindings, size_t first_drawn_vertex, size_t drawn_vertex_count, RenderOrder order) const;
		void DrawIndexed(ResourceHandle shader, ResourceHandle index_buffer, std::vector<ResourceBinding> bindings, size_t first_drawn_index, size_t drawn_index_count, RenderOrder order) const;
